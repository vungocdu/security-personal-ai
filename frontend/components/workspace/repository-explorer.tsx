    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-[0.26em] text-slate">Repository</p>
          <p className="mt-1 truncate text-sm font-semibold text-ink">Firebase Storage Explorer</p>
        </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3">
        ...
        <div className="min-h-0 flex-1 overflow-hidden rounded-2xl border border-line/70 bg-white/70" data-testid="repository-folder-tree">
          <ScrollArea className="h-full">
            <div className="p-2">
              {loading ? (
                <div className="rounded-xl bg-mist px-3 py-3 text-sm text-slate">Loading repository...</div>
              ) : (
                <FolderTree
                  rootPath=""
        ...
        <div className="min-h-0 flex-1 overflow-hidden rounded-2xl border border-line/70 bg-white/70" data-testid="repository-contents">
          <ScrollArea className="h-full">
            <div className="p-2">
              {!currentListing ? (
                <div className="rounded-xl bg-mist px-3 py-3 text-sm text-slate">Select a folder to view contents.</div>
              ) : (
                <div className="space-y-1">
                  ...
                  {currentListing.folders.length === 0 && currentListing.files.length === 0 ? (
                    <div className="rounded-xl bg-mist px-3 py-3 text-sm text-slate">This folder is empty.</div>
                  ) : null}
                </div>
              )}
            </div>
...
function FolderTree(props: {
  ...
  return (
    <div className="space-y-1">
      ...
      {listing.folders.length === 0 ? <div className="px-2 py-2 text-sm text-slate">No folders yet.</div> : null}
    </div>
  );
}
