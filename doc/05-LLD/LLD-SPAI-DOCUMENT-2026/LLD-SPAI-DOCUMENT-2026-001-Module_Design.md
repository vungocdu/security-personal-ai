...
- **Objective:** Dinh nghia logical document, mandatory versioning model, upload/index status va metadata toi thieu phuc vu repository tree, preview va retrieval.
...
- **Business Rules:**
  ...
  - Version bi thay the phai giu lien ket `supersedes_version_id`.
...
- **Interfaces:**
  ...
  - `accept_upload(file, metadata) -> DocumentAcceptedResponse`
...
| `POST /api/v1/documents` | Frontend upload | FastAPI | REST multipart | `DocumentAcceptedResponse` | async ingestion |
...
- **Data Flow:** upload metadata -> logical document resolution -> version row -> chunk rows.
- **Retention & Archiving:** Logical document co the giu nhieu version; khong xoa hard-delete trong MVP baseline.

## 8. ERROR HANDLING & RESILIENCE

- **Failure Modes:** duplicate upload, version mismatch, missing current version, parse fail.
...
- **Unit Tests:** upload response shape, document listing defaults, version list semantics, not found handling.
- **Integration Tests:** upload -> accepted response; get document -> current version consistent.
...
| --- | --- | --- | --- | --- | --- |
| R-LLD-DOC-001 | Logical document va version bi lon xon neu khong co policy ro | High | Enforce mandatory versioning policy trong service + tests | Backend | Open |
